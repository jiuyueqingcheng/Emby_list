...
class WidgetAPI {
    async get(url, options) {
        ...
        try {
            ...
            if (!resp || 200 !== resp.statusCode) throw new Error(`7}2}1}5}: ${(null == resp ? void 0 : resp.statusCode) || "æªç¥éè¯¯"}`);
            return resp.data;
        } catch (error) {
            throw new Error(`1}C}7}2}1}5}: ${error instanceof Error ? error.message : "æªç¥éè¯¯"}`);
        }
    }
...
async function getList(params) {
    ...
    try {
        ...
        const list = Array.from($('.videos-text-align')).map((el)=>{
            ...
            if ($parent.length > 0) {
                console.debug("è·³è¿èç½");
                return null;
            }
            const link = $el.find('a').attr('href');
            if (!link) {
                console.debug("è·³è¿æ²¡æé¾æ¥çåç´ ");
                return null;
            }
